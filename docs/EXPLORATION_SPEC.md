...
- COMBAT SIGNAL v1:
  ...
  - 第5層一部 / 第6層主対象 / 第7層確認枠の敵に、予兆付き一時戦術を付与する
  - 予兆ターンで BATTLE CODE の `enemy_signal_*` 条件が成立する
  - 既存のターン上限、報酬、ボス抽選、CTは変更しない
...
- COMBAT SIGNAL が発生した戦闘では、戦利品の下に `敵戦術パターン` を短く要約する
...
- パーツ獲得がある、または所持数が上限の80%以上の場合は、次の行動でパーツ確認へ進みやすくする。

## 8. 陣営守護戦との関係
- 出撃処理では守護戦用の直接書き込みを行わない。
- 守護戦は既存の `world_events_log` を週次再集計して作る。
- 対象:
  - 出撃勝利: `audit.explore.end` から +1
  - ボス撃破: `audit.boss.defeat` から +30
  - 進化成功: `audit.part.evolve` から +15
- 管理者活動、出撃敗北、進化失敗は対象外。
- 反映先は `faction_guardian_attacks` と `faction_guardians.current_hp`。
- 報酬、戦闘ステータス補正、CT短縮、ドロップ率補正は追加しない。
...
- 起動試験1〜3回目は、第1層の通常抽選・tutorial forced・警報保証より先にボスを抑制し、通常敵と戦う。
- 起動試験完了と初回機体調整完了の両auditがあり、第1層ボス未遭遇・未撃破なら、次の第1層探索を既存ボス戦へ確定する。
- 第2層探索、validation/cooldown失敗では保証を消費しない。`audit.boss.encounter` に `boss_source=onboarding_first_boss` が保存された時点で消費済みとする。
- 初回確定ボスの敗北後は既存再挑戦、勝利後は既存報酬・第2層解放へ接続する。
...
- 3回完了報酬はコイン `100`。`users.onboarding_first_three_reward_claimed` で1アカウント1回に制限する。
- 第1層初勝利の結果画面だけ、主CTAを `もう一度、第1層へ出撃` にし、CT中も表示したままカウントダウン後に押せる状態へ戻す。
- 再出撃CTAの `entry_source` は `battle_retry`。

## 10. 第1層ボス再挑戦保証 v1
- 対象は第1層固定ボスのみ。第2層以降、第4/5層試験ボスには適用しない。
- 第1層ボスへ一度遭遇し、未撃破のユーザーは `POST /boss/retry/layer-1` から直接再挑戦できる。
- 再挑戦にも既存の共通出撃CTを適用する。管理者0秒、新規/課金ブースト20秒、通常40秒の定義は変更しない。
- ボスステータス、8ターン上限、命中/会心/ダメージ計算、報酬、DECOR重複抑止、第2層解放条件は通常遭遇時と同じ。
- 基地の `NEXT ACTION` は、初回出撃フォーカス後かつ再挑戦可能状態なら `第1層ボス再攻略` を優先表示する。
- リカバリー主CTAは `パーツを強化する`、`機体を調整する`、`第1層へ出撃する` の順で1つだけ選ぶ。再挑戦は副CTAとして残す。
- リカバリーから通常戦へ戻る場合は `entry_source=boss_recovery_normal` を使う。
- 敗北結果では `ボス解析完了`、簡易敗因、おすすめ行動、`今の機体で再挑戦する`、`基地へ戻る` の順に出す。
- 強化リカバリー経由の強化成功時だけ、強化結果に `ボスへ再挑戦する` を表示する。

## 11. 第2層解放・次目標導線 v1
- 対象は第1層固定ボス初撃破から第2層初出撃まで。
- 第1層固定ボス初撃破で既存条件どおり第2層を解放し、戦闘結果に `第1層 突破` カードを表示する。
- 結果カードの主CTAは `第2層へ進む`。`area_key=layer_2`, `entry_source=layer2_unlock_result` で既存 `/explore` へPOSTする。
- 共通CT、active robot確認、二重送信対策、第2層解放チェックは既存出撃処理を使う。
- 第2層未出撃中は基地 `NEXT ACTION` を `新区域 解放` / `第2層へ出撃` に切り替える。entry_source は `layer2_unlock_home`。
- 優先順位は第1層ボス再挑戦、 第2層解放直後、初回出撃、初回3出撃、機体更新、前回出撃先の順。
- 第2層初出撃後は専用NEXT ACTIONを終了し、通常の第2層探索導線へ戻す。
- 戦闘バランス、報酬、出撃CT、第2層解放条件は変更しない。
