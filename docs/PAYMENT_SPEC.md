...
## 7. 出撃CTとの競合整理
- 管理者: `0秒`
- 研究ブースト所持かつ自動使用ON: その出撃だけ1回分を消費して `0秒`
- 課金ブースト中: `20秒`
- 新規ブースト対象（登録後72時間）: `20秒`
- 通常: `40秒`
- 優先順位は `管理者 -> 研究ブースト -> 課金ブースト -> 新規ブースト -> 通常`
...
## 13.1 陣営ショップと既存コイン
- 陣営ショップは新通貨を導入せず、既存の `users.coins` を使う。
- 陣営ショップ商品は称号、バッジ、背景、記念品などの表示用アイテムに限定する。
- 課金で得たコインでも購入できる可能性があるため、攻撃力、防御力、HP、探索報酬、ボス勝率、観測塔結果に影響する商品は置かない。
- 新通貨「陣営メダル」は導入しない。
