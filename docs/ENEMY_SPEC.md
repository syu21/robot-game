...
## 8. 第1層ボス警報
- 第1層固定ボス未撃破の通常プレイヤーだけが対象。
- `layer_1` の通常敵に勝利するたび、`user_boss_progress.no_boss_streak` を1進める。
- 10到達で `ボス警報発令`。次の第1層出撃では通常抽選を行わず、第1層固定ボスを確定出現させる。
- 保証遭遇後は警報値を0へ戻す。ボス未撃破なら再び0から進める。
- 10到達前に通常抽選で第1層ボスへ遭遇した場合も警報値を0へ戻す。
- 第1層固定ボス撃破後は警報を停止し、UIにも表示しない。
- `audit.boss.encounter` payload には `encounter_source` を入れる。
  - 保証遭遇: `alert_guarantee`
  - 通常抽選: `random`
## 観測塔での敵利用
...
- 起動試験の正常出撃完了数が3未満の通常ユーザーには、第1層ボスを出現させない。
- 初回おすすめ換装完了後、対象コホートの次回第1層出撃では既存第1層ボスを確定する。
- ボス本体の能力、行動、報酬、撃破・再挑戦処理は通常の第1層ボスと同一。
- 第2層以降と既存ユーザーの通常ボス抽選には適用しない。
