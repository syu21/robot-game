...
- ログイン後に `/home` を開き、`今週のランキング` と `今週のMVP` で `小ロボが主役 / 補助アバターが小さく添う` 見え方になっているか確認
- ログイン後に `/world` を開き、MVP に `小ロボ主役 + 補助アバター + 機体画像` が出るか確認
- ログイン後に `/records` を開き、初達成記録 / 今週の記録 / 話題ロボに `小ロボ主役 + 補助アバター + 機体画像` が出るか確認
