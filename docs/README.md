...
- `/build` では同名・同強化値パーツも個体ごとに並べたまま、`現在装備との差分` を主役に比較できる
- `/admin/metrics` では `進行状況` セクションから、層到達人数 / 停止層 / ボス未撃破 / 最終活動を見返せる
- 課金導線は `創設支援パック 100円 / ラボ維持支援パック 300円 / 出撃ブースター 500円` の3商品に整理し、支援系は `名前横バッジ + 限定DECOR`、ブースターは `14日間 探索CT 40秒 -> 20秒` の利便性課金として扱う
- `PvP は入口ではなくゴール`
- 今後の提案は次の3条件で判断する
  - 周回が気持ちよくなるか
  - 競争が自然発生するか
