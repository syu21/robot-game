...
- `/support` で `支援する（100円）` ボタンから Stripe Checkout の支援パック購入に進める
- `support_pack_001` 完了時は DECOR `shien_trophy` に加えて `創設支援章` を `user_trophies` へ冪等付与し、支援した証をゲーム内に残す
...
- ヘッダー / `/comms/world` / `/comms/rooms` / `/ranking` / ホームのランキング・MVP / `/world` MVP / `/records` では、ユーザー名の近くに支援者トロフィーバッジを出せる
