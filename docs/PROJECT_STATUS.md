...
- `/explore` の結果前に、既存 turn_logs を圧縮した `1〜2秒台のショート戦闘演出` を差し込み、通常戦は短く・ボス戦だけ少し強める
- ショート戦闘演出は `RIGHT_ARM=主攻撃 / LEFT_ARM=防御・支え / HEAD=センサー点灯 / LEGS=踏み込み・回避` を CSS/JS で見せ、SKIP で即結果へ移れる
- ショート戦闘演出は `/admin/release` の `ショート戦闘演出` 公開フラグで、`管理者だけ先行確認 -> 一般公開` に切り替えられる
