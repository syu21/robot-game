...
4. ロボ編成で機体更新
5. ボス戦・層解放を進める
6. 今日の研究指令から軽い日次目的を選んで進める

補助ループ:
1. 基地でソロ研究タスクを確認
2. 出撃・勝利・パーツ入手・編成変更で研究を進める
3. 研究EXPと研究Lvを上げる
4. 研究ノートで完了研究と個人記録を見返す
...
- 初回3出撃の計測は成功した `audit.explore.end` の時系列を正本とし、過去ログも1回目・2回目・3回目を再構築する。今回追加したのは計測基盤のみで、CT・報酬・ボス・ドロップ・ゲーム進行は変更していない
- `audit.explore.end` は `sortie_index`, `onboarding_phase`, `entry_source` を記録し、管理画面ではCTAクリック・出撃開始・出撃完了を別指標として表示する
- 初回3出撃専用のゲーム体験（CT 0秒、パーツ保証、おすすめ換装、ボス抑制）は未実装の次フェーズ項目
...
- `ソロ研究所` カードを追加し、次に進める研究タスク、研究Lv、最近の個人記録、研究ノート導線を表示する
- `今日の研究指令` を Next Action 直下に3件まで表示し、出撃・強化・編成・探索傾向を日替わりで促す
- 研究指令は全員共通でJST 00:00に切り替わり、未解放区画は同テーマの第1層代替へ置き換える
- 第5〜7層到達者の研究指令は、到達済み高層区画の育成傾向を候補にできる
- 研究指令の報酬は少量コインと研究継続記録だけで、戦力インフレ報酬は配らない
- 週替わり挑戦機《異常個体》は release flag `anomaly` で管理し、管理者確認後に一般公開する
- 異常個体は初回3出撃後に解放され、到達層に応じて `観測級 / 実戦級 / 深層級` に分かれる
- 異常個体の報酬は週・級ごとの初回撃破小額コインのみ。ランキング、解析率、最短ターンを研究文化の主報酬にする
- 初回出撃前の通常ユーザーはホーム主CTAを `出撃準備完了` / `第1層へ出撃する` に統一し、案内モーダルや重複CTAを抑えて `next_action_first_explore` とホーム滞在秒数を探索ログへ残す
- 初回勝利後から3出撃未満まではホーム主CTAを第1層再出撃へ寄せ、初回3出撃の継続を促す
- 初回出撃フォーカス終了後は通常の Next Action / 初回3出撃導線へ戻る
- `Next Action` はたたむ/開くことができ、ボス警報中は自動で開く
- 初回ユーザーまたは未出撃ユーザーは `第1層へ出撃する` を最優先 CTA として出し、最初の1勝まで迷いにくくする
...
### 3.3 出撃（/explore）
- 1出撃1戦（暴走時のみ連戦あり）
- COMBAT SIGNAL v1 を追加し、第5層一部 / 第6層主対象 / 第7層確認枠の敵に予兆付き戦術を付与した
  - `OVERCHARGE / AEGIS / LOCK-ON / PHASE SHIFT`
  - 予兆ターンで BATTLE CODE 条件 `enemy_signal_*` が成立する
  - 戦闘ログ、短縮リプレイ、敵図鑑、管理者敵一覧へ表示する
...
- 所持パーツ: `/parts`
  - 装備中と所持中を同じ一覧で比較可能
  - `画像 / 部位 / レアリティ / +値 / 6ステ / 装備中表示 / 総合性能 / 注目能力` を表示
  - 部位フィルター `すべて / 頭 / 右腕 / 左腕 / 脚`
  - ソート `おすすめ順 / 新しい順 / 古い順 / 総合性能順 / +値順 / レアリティ順 / 部位順` を持ち、部位フィルターやページ送りと併用しても条件が落ちない
...
- ロボ編成: `/build` + `/build/confirm`
  - パーツ候補カードは `現在装備との差分` を主役にし、初期表示は `パーツ名 / +値 / 個体ID / 総合性能差分 / 注目2能力差分` に絞る
  - `6ステ実数 / レアリティ / 同名内の並び情報` は `詳細を開く` に退避し、スマホでも縦に伸びすぎないようにする
- ロボ個別ページから `機体整備` に入れ、HEAD / 右腕 / 左腕 / 脚 / DECOR を1部位ずつ差し替えながら `総合性能差分 / 6ステ差分 / 思想変化` を見て確定できる
...
- 固定ボス16体
  ...
  - layer_4_final
  - layer_5_reboot / layer_5_overdrive
  - layer_5_final
  - layer_6_rebuild / layer_6_core
  - layer_6_final
  - layer_7_echo / layer_7_chaos
  - layer_7_final
...
- 研究モジュール拡張 Phase 3: `戦闘命令構築 / BATTLE CODE` を追加。v2 DUAL LOGICではLOGIC A/Bを持ち、先に成立した命令を1度だけ実行する。詳細は `docs/MODULE_BATTLE_CODE_SPEC.md`。
- BATTLE CODE LIBRARY v2: BATTLE CODEを3枠保存し、SINGLE/DUAL CODEをホーム/ライブラリ画面から切り替え可能。詳細は `docs/MODULE_BATTLE_CODE_LIBRARY_SPEC.md`。

## 11. 第1層 初回体験改善
- 第1層初勝利の結果画面に専用案内と主CTA `もう一度、第1層へ出撃` を追加。
- CT中も再出撃CTAを表示し、既存CT値でカウントダウン後に有効化。
- 初回3出撃進捗を基地/結果画面へ表示し、3回完了時にコイン100を1回だけ付与。
- 初回3出撃後は基地/結果/パーツ/編成で `解析完了：交換可能なパーツを検出` を出し、最終ステータス計算で改善する所持パーツを1件推奨する。
- 推奨パーツは自動装備しない。`/build?guide=first_upgrade` で所有・在庫・部位・改善値を再検証し、交換候補だけ軽くハイライトする。
- 初回更新完了後は `/robots` に `機体更新完了！` と `新しい機体で出撃する` を表示し、`entry_source=first_robot_upgrade_result` で再出撃を計測する。
- 第1層固定ボス未撃破ユーザー向けに、通常勝利10回で次回ボス保証となる警報ゲージを追加。
- 管理者、集計除外ユーザー、ボス撃破済みユーザーは対象外。

## 12. ホーム出撃導線統合 v2
- `/home` 上部を `最近の研究機体`、統合 `NEXT ACTION`、`出撃機体` の順に整理。
- ホーム上の主出撃CTAは統合 `NEXT ACTION` 1枚に集約し、前回出撃先、ボス警報、パーツ満杯警告、層別目標を同カードへ表示。
- 出撃機体カードはロボ画像、名前、総合、勝利数、現在のモジュール、変更導線を中心に簡略化。
- 監査は `audit.home.next_action.view` / `audit.home.next_action.click` / `audit.home.quick_start` / `audit.home.previous_area.click`。
- 管理メトリクスに 15/30/60秒以内出撃、NEXT ACTIONクリック率、ホーム滞在中央値、ホーム直接出撃率、前回出撃先利用率を追加。

## 13. 第1層ボス再挑戦保証 v1
- 第1層固定ボスに遭遇済みかつ未撃破なら、基地/敗北結果から `POST /boss/retry/layer-1` で直接再挑戦可能。
- 状態は `user_boss_progress` の再挑戦カラムで管理する。新規テーブルは追加しない。
- 再挑戦にも共通出撃CTを適用。ボスステータス、報酬、DECOR重複抑止、第2層解放条件は変更なし。
- 基地 `NEXT ACTION` は再挑戦可能状態を `第1層ボス再攻略` として優先する。主CTAは `パーツを強化する`、`機体を調整する`、`第1層へ出撃する` の順で1つだけ選び、`ボスへ再挑戦` は副CTAとして残す。
- 敗北結果には `ボス解析完了`、簡易敗因、おすすめ行動、`今の機体で再挑戦する` を出す。
- `POST /boss/retry/layer-1/build` から既存 `/build?guide=boss_retry` に接続し、命中/耐久/火力診断に合う総合性能上昇パーツを `再挑戦おすすめ` 表示する。
- 調整保存後はロボ一覧の結果カードに `再挑戦準備完了` と `第1層ボスへ再挑戦` を表示する。再挑戦状態の消費タイミングは変更しない。
- 強化リカバリー経由で強化成功した場合だけ、強化結果に `ボスへ再挑戦する` を表示する。
...
## 15. 第6層 攻略記録・研究文化 v1
- 第6層公開後の「到達して終わり」を避けるため、既存 `audit.explore.end` / `audit.boss.defeat` から攻略記録を集計する。
- `/records` に `第6層研究記録` を追加。最短攻略、安定攻略、研究熱、最終試験初撃破を最大3件ずつ表示する。
- `/ranking?metric=layer6_research` に今週の第6層通常戦データ収集回数ランキングを追加する。
- ホームの既存ランキング領域に `第6層研究速報` を短く表示する。NEXT ACTION より上には出さない。
- DBテーブル追加なし。二重送信は集計時に `request_id`、次に `battle_id` で重複排除する。
- 管理メトリクスに第6層到達者、出撃数、勝率、平均ターン、ボス遭遇/撃破、記録掲載者を追加する。

## 16. 週替わり挑戦機《異常個体》v1
- `/anomaly` に週替わりの一人用挑戦画面を追加。既存チャンピオン戦闘を再利用し、最大8ターンで撃破のみをクリア扱いにする。
- テンプレートは `VEIL RUNNER / GRAVEMASS / REDLINE / PARADOX` の4系統。毎週1体を決定的に生成する。
- 級は最大到達層で `観測級(1-2層) / 実戦級(3-4層) / 深層級(5層以上)` に分離する。
...
## 22. 表示数値インフレ化 v1
- プレイヤー向けのロボ6ステ、総合性能、パーツ6ステ、総合性能、敵図鑑ステータス、戦闘HP/ダメージ表示を内部値の100倍で表示する。
...
## 25. モジュール研究システム Phase 1-2
- 既存モジュール合成結果は変更せず、成功時に `module_fusion_records` / `module_fusion_inputs` へ結果と素材の表示snapshotを保存する。
- 消費済み素材も研究記録から投入名、世代、主系統を確認できる。
- `/modules/<module_id>/lineage` で本人所有モジュールの系譜タイムラインを表示する。
- `/modules/research` で合成実験数、観測済み系統、最高確認世代、初観測、自己最高世代更新を確認できる。
- 通常カードの長い由来は圧縮表示し、詳細は系譜画面へ逃がす。
- 内部確率、重み、score、seed、レシピ推定はUIに表示しない。

## 26. モジュール研究システム Phase 4 観測帳
- `/modules/research` に `実験記録 / 観測帳` タブを追加。
- 観測帳は `module_fusion_records` を正本に、本人が結果として生成した主系統だけを `GROUP BY` 集計する。
- 系統ごとに観測回数、初観測、最終観測、最高確認世代を表示し、詳細では該当実験をページング表示する。
- 素材投入だけでは観測扱いにせず、結果系統が空のlegacyは推測しない。結果系統が確定しているlegacyは観測として扱う。
...
## 28. モジュール研究システム Phase 6 追試結果共有
- `追試したい` を押した他人の共有研究に、自分の後続合成記録を `追試結果` として関連付け可能。
- 候補は直近7日以内の `replicate` reaction、最大5件、本人の共有研究と削除済み投稿は除外。
- 子記録は本人所有の正常な合成記録のみ。既に別親へ登録済みの子記録、legacy、他人の記録は拒否。
- 子記録が既に共有済みなら既存共有を再利用し、未共有なら通常の研究共有を作成してから親子関係を保存。
- 公開研究詳細、最近の研究、会議室カードに直接の元研究/追試数/子追試を最小表示。
- 管理メトリクスに追試登録数、追試登録ユーザー数、追試された元研究数、`追試反応 -> 追試共有` の行動相関を追加。
- 成功率、再現率、レシピ一致、自動追試、報酬、合成アルゴリズム変更は追加なし。

## 29. 起動試験 3出撃
- `audit.explore.end` が3件未満の通常ユーザーは、Homeと戦闘結果で「起動試験」0/3〜3/3を表示する。
- 起動試験中の出撃CTは0秒。3回完了後は新規72h、課金ブースト、通常CTの既存優先順位へ戻る。
- 3回目終了時、所持中に現在装備より総合性能が高い候補がなければNパーツを1個保証する。所持上限時はoverflowへ保管する。
- 完了結果から `/onboarding/adjust` へ進み、既存build modify処理でおすすめ1部位だけを換装できる。
- 換装後は大型反応CTAから第1層へ再出撃する。
- 起動試験1〜3回目はサーバ側の敵決定で第1層ボスを抑制する。ボス遭遇auditや保証消費は発生させない。
- 起動試験完了auditと初回調整完了auditを持つ未遭遇ユーザーは、次の第1層出撃で既存第1層ボスへ1回だけ確定遭遇する。
- 調整完了画面は「大型反応へ出撃」を主CTAとし、`entry_source/boss_source=onboarding_first_boss` で計測する。
