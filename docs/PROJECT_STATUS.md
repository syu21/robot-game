...
- battle-cinematic-v1 のレーザーは `attack_element -> 攻撃側element/parts -> neutral` の順で `fire / water / thunder / wind / dark / light / neutral` を出し分ける。演出専用で戦闘計算・通常戦8ターン制・turn_logs・audit.*互換には影響しない
...
- 通常敵は8ターン上限を維持し、数値の大きさではなく `型相性 + 圧力差` で難度を作る
- ボス戦のみ8ターン上限を撤廃し、内部安全上限50ターンで未決着時は試験継続不能扱いにする
