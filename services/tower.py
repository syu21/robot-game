...
TOWER_ENVIRONMENTS = (
    {
        "key": "stable_week",
        "display_name": "安定観測週",
        "description": "命中と耐久が記録を伸ばしやすい観測環境です。",
    ...
    },
    {
        "key": "speed_week",
        "display_name": "機動観測週",
        "description": "素早さと短期突破が評価されやすい観測環境です。",
    ...
    },
    {
        "key": "power_week",
        "display_name": "暴走観測週",
        "description": "攻撃と会心が伸びやすい一方、事故も起きやすい観測環境です。",
    ...
    },
    {
        "key": "heavy_week",
        "display_name": "重装観測週",
        "description": "長期戦になりやすく、防御と耐久が重要な観測環境です。",
    ...
    },
    {
        "key": "mist_week",
        "display_name": "霧界観測週",
        "description": "命中不足を咎める敵が多い観測環境です。",
...
def get_current_tower_environment(now_ts=None):
    week_key = current_week_key(now_ts)
    week_num = int(week_key.split("W")[-1])
    env = dict(TOWER_ENVIRONMENTS[week_num % len(TOWER_ENVIRONMENTS)])
...
def create_tower_run(db, user_id, robot_ids, *, now_text=None, seed=None, environment_key=None):
    ...
    env_key = environment_key or get_current_tower_environment()["key"]
    ...
    cur = db.execute(
        ...
        (user_id, squad_robot_1_id, squad_robot_2_id, squad_robot_3_id, started_at, start_floor,
         current_floor, reached_floor, max_floor_in_run, status, environment_key, seed, result_summary_json, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, 1, 1, 0, ?, 'active', ?, ?, ?, ?, ?)
        """,
        (
            int(user_id),
...
def get_tower_rankings(db, *, weekly_key=None, limit=20):
    ...
    return {"weekly_key": week_key, "weekly": weekly, "all_time": all_time}
