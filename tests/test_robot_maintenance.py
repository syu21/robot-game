...
class RobotMaintenanceTests(unittest.TestCase):
    ...
    def test_robot_decompose_blocks_when_returned_parts_exceed_inventory_limit(self):
        ...
        with client.session_transaction() as sess:
            self.assertEqual(
                sess.get("message"),
                "所持パーツがいっぱいのため、このロボは分解できません。先に不要なパーツを整理してください。",
            )
        ...
        with game_app.app.app_context():
            ...
            audit_row = db.execute(
                ...
                SELECT payload_json
                FROM world_events_log
                WHERE user_id = ? AND event_type = 'audit.robot.decompose.blocked'
                ORDER BY id DESC
                LIMIT 1
                """,
                (self.user_id,),
            ...
            self.assertEqual(int(payload.get("return_count") or 0), 4)
