...
class LabCasinoRouteTests(unittest.TestCase):
    ...
    def test_lab_race_daily_grant_applies_once_per_day(self):
        ...
        with game_app.app.app_context():
            ...
            self.assertEqual(int(row["coins"]), 40)
    ...
    def test_lab_race_bet_resolves_race_and_writes_audit(self):
        ...
        resp = client.post(
            "/lab/race/bet",
            data={"race_id": int(race["id"]), "entry_id": int(entry["id"]), "amount": 20},
    ...
    def test_lab_race_prize_claim_is_disabled_while_prizes_are_preparing(self):
        ...
        with game_app.app.app_context():
            ...
            self.assertEqual(int(wallet["coins"]), 40)
    ...
    def test_lab_home_shows_single_enemy_race_card(self):
        ...
        self.assertEqual(resp.status_code, 200)
        html = resp.get_data(as_text=True)
        self.assertIn("本編の外側で、いろいろ試している場所", html)
        self.assertIn("投稿ロボを見たり、レースを眺めたり、小さなロボを育てたりできます。", html)
        self.assertIn("敵ロボ6体のレースを予想して遊ぶ", html)
        self.assertIn("AIロボ生成", html)
        self.assertIn("研究所AIでロボを作る。作った画像は実験室へ投稿できます", html)
        self.assertIn('/lab/ai-robot-generate?source=lab_top', html)
        self.assertIn('target="_blank"', html)
        self.assertIn('rel="noopener noreferrer"', html)
        self.assertIn("作ったロボを実験室へ投稿する", html)
        self.assertIn("みんなのロボを見る", html)
        self.assertIn("自分の投稿ロボを確認する", html)
        self.assertIn("レース結果と予想履歴を見る", html)
        self.assertIn("エネミーレース", html)
        self.assertLess(html.index("本編の外側で、いろいろ試している場所"), html.index("注目の投稿ロボ"))
        self.assertLess(html.index("注目の投稿ロボ"), html.index("エネミーレース"))
        self.assertLess(html.index("エネミーレース"), html.index("AIロボ生成"))
        self.assertLess(html.index("AIロボ生成"), html.index("ロボ投稿"))
        self.assertLess(html.index("ロボ投稿"), html.index("投稿ロボ展示"))
        self.assertLess(html.index("投稿ロボ展示"), html.index("自分の研究機体"))
