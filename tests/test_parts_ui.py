...
class PartsUiTests(unittest.TestCase):
    ...
    def test_parts_inventory_comparison_filter_and_safe_pagination(self):
        ...
        self.assertIn("使わないパーツは", html)
        self.assertIn("robo-fixed-nav", html)
        self.assertIn("parts-bottom-actions", html)
        self.assertIn("パーツ強化へ", html)
        self.assertIn("廃品市場へ", html)
        self.assertNotIn("完全削除（コインなし）", html)
    ...
    def test_parts_page_hides_legacy_storage_and_excludes_it_from_usable_candidates(self):
        ...
        self.assertIn("使わないパーツは", html)
        self.assertNotIn("所持上限を超えた新規戦利品は自動売却されます。", html)
    ...
    def test_part_lock_redirect_uses_safe_return_to(self):
        head_part = self._create_custom_part("HEAD", "lock_safe_return_head", "保護戻り先ヘッド")
        self._create_extra_instance(head_part, plus=0, status="inventory")
        with game_app.app.app_context():
            db = game_app.get_db()
            part_id = int(
                db.execute(
                    ...
                    LIMIT 1
                    """,
                    (self.user_id, "lock_safe_return_head"),
                ).fetchone()["id"]
            )
        ...
        resp = client.post(
            f"/parts/{part_id}/lock",
            data={"return_to": "/parts?part_type=HEAD&sort=part_type&page=2#part-1"},
            follow_redirects=False,
        )
        self.assertEqual(resp.status_code, 302)
        self.assertIn("/parts?part_type=HEAD&sort=part_type&page=2#part-1", resp.headers.get("Location", ""))

        resp = client.post(
            f"/parts/{part_id}/unlock",
            data={"return_to": "https://example.com/phishing", "part_type": "HEAD", "sort": "part_type"},
            follow_redirects=False,
        )
        self.assertEqual(resp.status_code, 302)
        location = resp.headers.get("Location", "")
