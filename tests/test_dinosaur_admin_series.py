...
class DinosaurAdminSeriesTests(unittest.TestCase):
    ...
    def test_dinosaur_n_parts_seeded_as_admin_only(self):
        with game_app.app.app_context():
            db = game_app.get_db()
            rows = db.execute(
                """
    ...
    def test_normal_user_does_not_receive_dinosaur_parts(self):
        with game_app.app.app_context():
            db = game_app.get_db()
            count = int(
                db.execute(
    ...
    def test_dinosaur_parts_are_excluded_from_normal_drops(self):
        with game_app.app.app_context():
            db = game_app.get_db()
            db.execute("UPDATE robot_parts SET is_active = 0 WHERE series NOT LIKE 'dino_%'")
            db.commit()
            dropped = game_app._pick_drop_part_master(db, rarity="N", area_key="layer_5_final", user_id=self.user_id)
