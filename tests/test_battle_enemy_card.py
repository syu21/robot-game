...
class BattleEnemyCardTests(unittest.TestCase):
    def setUp(self):
        ...
        with game_app.app.app_context():
            ...
            db.execute(
                ...
                VALUES (?, ?, 'active', ?, ?)
                """,
                (self.user_id, "EnemyCardBot", now, now),
            )
            robot_id = db.execute(
    ...
    def test_explore_result_contains_enemy_card_with_image(self):
        with game_app.app.test_client() as client:
            with client.session_transaction() as session:
                session["user_id"] = self.user_id
                session["username"] = "enemy_card_tester"
